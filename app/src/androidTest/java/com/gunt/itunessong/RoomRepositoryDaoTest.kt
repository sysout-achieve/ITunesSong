...
class RoomRepositoryDaoTest {
    ...
    fun trackDao1InsertTest() {
        // when
        appDatabase.trackDAO().insert(trackEntity).blockingGet()

        // then
        val list = appDatabase.trackDAO().getAll().blockingGet()
        assertThat(list[0]).isEqualTo(trackEntity)
    }
    ...
    fun trackDao4FetchTest() {
        ...
        // 저장된 31개의 데이터 중에 limit 10 offset 30일 경우 -> listSize 기대값 = 1
        val expected = 1
        val resultList = appDatabase.trackDAO().fetchTracks(10, 30).blockingGet()
        assertThat(resultList.size).isEqualTo(expected)
    }
